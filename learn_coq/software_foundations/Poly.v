...
Definition hd_error {X : Type} (l : list X) : option X := nth_error l 0.
...
Proof.
  intros X Y f l m. induction l as [|h' l' IHl'].
  - simpl. reflexivity.
  - simpl. rewrite -> IHl'. reflexivity.
Qed.
...
Example fold_allzeros := fold (fun n => andb (beq_nat 0 n)) [0;0;0] true = true.
(** [] *)
...
Definition succ (n : nat) : nat := fun (X : Type) (f : X -> X) (x : X) => n _ f (f x).
