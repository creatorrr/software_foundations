...
Proof.
  intros [] m p.
  - reflexivity.
  - replace (S n + (m + p)) with ((m + n) + S p).
    + replace (S n + p) with (n + S p).
      * rewrite -> plus_assoc. reflexivity.
      * rewrite <- plus_n_Sm. rewrite -> plus_Sn_m. reflexivity.
    + rewrite <- plus_n_Sm. rewrite -> plus_Sn_m. rewrite -> plus_assoc.
