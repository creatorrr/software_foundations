...
Module NatList.
  ...
  Fixpoint oddmembers (l:natlist) : natlist :=
    match l with
    | nil => nil
    | h :: tail => if oddb h then (h :: (oddmembers tail)) else (oddmembers tail)
    end.
  ...
  Fixpoint alternate (l1 l2 : natlist) {struct l1} : natlist :=
    match l1 with
    | nil => l2
    | h1 :: tail1 => match l2 with
                    | nil => l1
                    | h2 :: tail2 => h1 :: h2 :: (alternate tail1 tail2)
                    end
    end.
  ...
  Fixpoint count (v:nat) (s:bag) {struct s} : nat :=
    match s with
    | nil => 0
    | h :: tail => bool_to_nat (beq_nat v h) + count v tail
    end.
  ...
  (* Defined here like this because it makes the Theorem bag_theorem *)
  ...
  Fixpoint remove_one (v:nat) (s:bag) {struct s} : bag :=
    match s with
    | nil => nil
    | h :: tail => if beq_nat v h then tail else h :: (remove_one v tail)
    end.
  ...
  Fixpoint remove_all (v:nat) (s:bag) {struct s} : bag :=
    match s with
    | nil => nil
    | h :: tail => if beq_nat v h then (remove_all v tail) else h :: (remove_all v tail)
    end.
  ...
  Fixpoint subset (s1:bag) (s2:bag) {struct s1} : bool :=
    match s1 with
    | nil => true
    | h :: tail => if member h s2 then andb true (subset tail (remove_one h s2)) else false
    end.
  ...
  Proof.
    intros. induction b as [nil | h tail IHtail].
    - assert (H1: count n [] = 0). reflexivity.
      assert (H2: add n [] = [n]). reflexivity.
      assert (H3: count n [n] = 1). {
      ...
      reflexivity.
    - assert (H4: add n (h :: tail) = h :: (add n tail)). { reflexivity. }
                                                          rewrite -> H4. simpl. rewrite -> IHtail.
      rewrite -> plus_n_Sm.
      reflexivity.
  Qed.
  ...
  Fixpoint beq_natlist (l1 l2 : natlist) {struct l1} : bool :=
    ...
    | h1 :: tl1 => match l2 with
                  | nil => false
                  | h2 :: tl2 => andb (beq_nat h1 h2) (beq_natlist tl1 tl2)
                  end
    end.
  ...
  Theorem count_nil_r: forall (p : nat), count p [] = 0.
  Proof. reflexivity. Qed.

  Theorem count_distr_cons: forall (p q: nat) (s : bag), count p (q :: s) = count p s + count p [q].
  Proof.
    intros p q s. destruct s as [| k s'].
    - rewrite -> count_nil_r. reflexivity.
    - simpl. rewrite <- plus_n_O.
      rewrite -> plus_comm.
      reflexivity.
  Qed.

  Theorem count_distr_app: forall (p : nat) (s t : bag), count p (s ++ t) = count p s + count p t.
  Proof.
    intros p s t. induction s as [| k s' IHs'].
    - reflexivity.
    - simpl. rewrite -> IHs'.
      rewrite -> plus_assoc.
      reflexivity.
  ...
  Theorem count_elem_swap: forall (x y z: nat) (b : bag), count x (y :: z :: b) = count x (z :: y :: b).
  Proof.
    intros x y z b. simpl. rewrite -> plus_swap. reflexivity.
  Qed.

  Theorem bag_count_sum_comm: forall (b c : bag) (n : nat),
      count n (sum b c) = count n (sum c b).
  Proof.
    intros b c n. destruct b as [| m b'].
    - rewrite -> app_nil_r. reflexivity.
    - rewrite -> count_distr_app.
      rewrite -> plus_comm.
      rewrite -> count_distr_app.
      reflexivity.
  Qed.

  Theorem bag_count_sum: forall (b : bag) (n : nat),
      count n (sum b b) = (count n b) * 2.
  Proof.
    intros b n. destruct b as [| m b'].
    - reflexivity.
    - rewrite -> count_distr_app.
      rewrite -> mult_comm.
      simpl. rewrite <- plus_n_O.
...
(There is a hard way and an easy way to do this.) *)
  ...
  Theorem eq_comm_natlist: forall (l1 l2: natlist), l1 = l2 -> l2 = l1.
  Proof.
    intros l1 l2 H.
    rewrite -> H.
    reflexivity.
